...
app.get('/api/health',(req,res)=>res.json({ok:true,version:'38.0.0',source:process.env.SOURCE_URL||'https://loungefinder.loungekey.com/en/linkcarevn/',time:new Date().toISOString()}));
app.get('/healthz',(req,res)=>res.status(200).json({ok:true,version:'38.0.0'}));
...
app.get('/api/search',async(req,res)=>{
  // V38: hard interactive deadline. Never fail a search only because VI translation
  // is still finishing. Return lounge results within the deadline and complete VI in background.
  ...
  let coreViPending=cacheNeedsCoreViRepair(cachedLounges);
  if(viPending && req.query.translate!=='0'){
    // Give translation a short foreground window so most searches return VI+EN together,
    // but never spend the whole 1-minute SLA waiting on a third-party translator.
    const budget=Math.min(18000,Math.max(1200,remaining()-2500));
    try{
    ...
    coreViPending=cacheNeedsCoreViRepair(cachedLounges);
  }
  // If any VI text is still missing, continue silently in the background.
  // Search results must still be returned so users never get stuck behind translation.
  if(viPending || coreViPending) translateAirportInBackground(code);
  ...
  res.json({airport:{...a,terminals:[...new Set([...(a.terminals||[]),...lounges.map(x=>x.terminal).filter(Boolean)])]},lounges,query:{datetime,terminal:req.query.terminal||'all',scope,onlyAvailable},refreshed,refreshError,cacheMode:(firstLookup||repairIncompleteCache)?'fast-complete':'fast-cache',hydrating,translationPending:Boolean(viPending||coreViPending)});
});
...
const port=Number(process.env.PORT||3000); app.listen(port,()=>console.log(`Lounge Finder V38 (fast results, VI+EN, <= 1 minute) running at http://localhost:${port}`));
